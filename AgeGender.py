...
parser.add_argument('--camera', action='store_true', help='Also run live camera after processing the input image (or run camera only if no input).')
parser.add_argument('--output', help='Path to save annotated image when processing a single image.')
...
if args.input:
    ...
    if img is None:
        ...
        t = time.time()
        out_img = process_frame(img, padding=20)
        print("time : {:.3f}".format(time.time() - t))
        if args.output:
            cv.imwrite(args.output, out_img)

    # If user didn't request camera, exit after processing image
    if not args.camera:
        print("Done processing image.")
        cv.waitKey(0)
        exit(0)

# Otherwise (no input or user requested camera), run live camera
...
while cv.waitKey(1) < 0:
    ...
    out_frame = process_frame(frame, padding=padding)
    print("time : {:.3f}".format(time.time() - t))
